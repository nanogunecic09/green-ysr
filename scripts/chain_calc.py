...
class chain():
    def __init__(self,N,pitch_x,direction=(1,0),U=0,J=0.04,spiral = 0,mode=0,m=18.7,pf = 0.21,delta_s = 0.78e-3,gamma_s=20e-6) -> None:
        self.N = N
        ...
        self.E = np.linspace(-5*self.delta_s,5*self.delta_s,1000)
    ...
    def didv_conv(self,coord,Delta_t,Gamma_t):
        dos = self.didv(coord)
        self.V = np.linspace(-self.delta_s*5,self.delta_s*5,1000)
    ...
    def LSconvolute(self,Delta_t,Gamma_t):
        ...
        self.V = np.linspace(-self.delta_s*2.8,self.delta_s*2.8,1000)
